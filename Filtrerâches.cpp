...
void trierTaches(Tache taches[], int nombreTaches, int ordre) {
    for (int i = 0; i < nombreTaches - 1; i++) {
        for (int j = 0; j < nombreTaches - i - 1; j++) {
            // Comparaison des dates en fonction de l'ordre selectionne
...
// echanger les taches si elles ne sont pas dans le bon ordre
                Tache temp = taches[j];
                taches[j] = taches[j + 1];
                taches[j + 1] = temp;
            }
...
void filtrerParPriorite(Tache taches[], int nombreTaches) {
    int prioriteFiltre;
    printf("Entrez la priorite a filtrer (1-5) : ");
    ...
    for (int i = 0; i < nombreTaches; i++) {
        if (taches[i].priorite == prioriteFiltre) {
            afficherTaches(taches + i, 1); // Affiche seulement la tache avec la priorite filtree
            trouve = 1;
        }
    }
...
int main() {
    ...
    do {
        ...
        switch (choix) {
            ...
            default:
                if (choix != 7) {
                    printf("Entrez un choix entre (1-7) :\n");
                }
        }
